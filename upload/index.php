    ...
    <div class="container" style="margin-top: 50px; padding-bottom: 10px; padding-top: 10px;">
        ...
        <form action="../upload_dwarf.php" , method="post" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <select class="form-select" id="type" name="type" onchange="onChose()" required>
                    <option disabled selected value> -- Urkunde auswählen --</option>
                    <?php
                    foreach (getDataTable($pdo, 'SELECT * FROM `atn_allowed` ORDER BY `section`,`dlrgid`,`sort`;') as $cert){
                ...
                <script>
                    function onChose(){
                        if(document.getElementById('type').value == "sonstige Ausbildung" || document.getElementById('type').value == "sonstige externe Ausbildung (HiOrg)") {
                            document.getElementById('customname').style.visibility = 'visible';
                            document.getElementById('customnamef').required = true;
                        } else {
                            document.getElementById('customname').style.visibility = 'hidden';
                            document.getElementById('customnamef').required = false;
                        }
                    }
                </script>

                <span id="customname" style="visibility: hidden">
                    <label for="formFile" class="form-label mt-4">Name der Ausbildung</label>
                    <input type="text" class="form-control" id="customnamef" name="customnamef">
                </span>
